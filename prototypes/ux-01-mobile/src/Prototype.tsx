...
export default function Prototype(){
  ...
  const reset=()=>{setScreen("home");setHistory([]);setStep(0);setDraft("");setIntake("");setAnswers([]);setQuestionOrder([]);setAllergy("");setSheet(null)};
  ...
  const Home=()=> <main className="home home-approved">
    ...
    <nav className="approved-home-nav" aria-label="主导航">
      <button aria-label="首页" onClick={()=>setScreen("market")}/><button aria-label="选宠" onClick={()=>setScreen("home")}/><button aria-label="社区" onClick={()=>setScreen("states")}/><button aria-label="我的" onClick={()=>setScreen("states")}/>
    </nav>
  </main>;
  ...
  const Chat=()=> {const q=questions[questionOrder[step]??0];return <main className="page chat-page">
    <div className="chat-inline-head"><button aria-label="返回" onClick={back}><ArrowLeftIcon/></button><strong>聊聊你的生活</strong><span/></div>
    {!intake?<>
  ...
  const Profile=()=> <main className="page"><em className="tag">你的相处画像</em><h2>我目前这样理解你</h2><p className="intro">这是聊天中的暂时整理，不是给你贴标签。你随时都能修改。</p><div className="profile">
    {[["独处时间","约5～8小时","已确认"],["陪伴投入","每天约1～2小时","已确认"],["互动节奏","喜欢适度贴贴","已确认"],["愿意调整","可以调整一些","已确认"],["掉毛接受度","还没有聊到","待确认"]].map(r=><div key={r[0]}><span><small>{r[0]}</small><strong>{r[1]}</strong></span><i className={r[2]==="待确认"?"pending":""}>{r[2]}</i></div>)}
  ...
  return <div className="gulu-shell" style={{"--gulu-status-height":device.platform==="ios"?"54px":"72px","--gulu-safe-area":device.platform==="ios"?device.geometry.safeArea.bottom+"px":"0px"} as CSSProperties}>
    ...
    {!["home","profile","allergy","edit","expired","error"].includes(screen)&&<nav className="nav"><button className={screen==="market"?"on":""} onClick={()=>setScreen("market")}><HomeIcon/><small>首页</small></button><button className={["home","chat","results"].includes(screen)?"on":""} onClick={()=>setScreen("home")}><ChatBubbleIcon/><small>选宠</small></button><button onClick={()=>setScreen("states")}><HeartIcon/><small>社区</small></button><button onClick={()=>setScreen("states")}><PersonIcon/><small>我的</small></button></nav>}
    ...
    <BottomSheet open={sheet==="reset"} onOpenChange={v=>!v&&setSheet(null)} title="要重新认识一次吗？" description="重置后，之前的回答和推荐不会带到新会话。"><div className="sheet"><button className="danger" onClick={reset}>确认重置</button><button onClick={()=>setSheet(null)}>先不重置</button></div></BottomSheet>
  </div>
}