...
- 版本：0.3.12
- 创建日期：2026-08-04
- 输入：`specs/SDD_SPEC.md` 0.3.12、`specs/IMPLEMENTATION_PLAN.md` 0.2.14
- 状态：UX-01最终版已批准；T06R至T44和T35B均已完成；T44已通过用户验收
- 当前事实：统一模型契约、确定性Mock、LangGraph显式流程、动态追问、知识检索、双层推荐和风险说明已串联；三类5轮以上Mock对话已稳定复现
- 门禁：T35真人演示与Mock证据已分开记录；Phase 8检查点V已通过用户验收，推送与部署仍须另行批准
...
- RED：先新增6条指标与运行器契约测试，首次因`evaluation.metrics`不存在而无法收集；指标层GREEN后，首次命令行运行又暴露直接启动时包路径缺失，以及5个固定案例没有一一对应回归测试。
- GREEN：新增确定性评测运行器、11项Gate 5指标计算、逐案例测试映射、JSON原始结果和Markdown报告；为狗过敏、叫声底线、语气强度不加分、品种与体型不重复计分、候选不足不凑数补齐5条固定回归测试。
- 真实执行：36条固定案例中34条由38条生产业务回归测试执行并全部通过，Bad Case为0；`EVAL-034`与`EVAL-035`涉及手机安装能力，明确延期到T42真实浏览器审查，不用后端测试冒充设备验证。
- 指标：关键硬约束覆盖率100%、硬约束违规推荐数0、已确认信息重复询问率0%、冲突识别召回率100%、下一问题规则一致率100%、档案引用完整率100%、推荐依据完整率100%、公共精确分泄漏数0、Mock网络调用数0；真人结构化输出成功率本次不适用。
- 边界：原型按钮反馈率只统计固定集已标注的联系商家按钮，全站明显按钮覆盖留给T42；本次不读取DeepSeek Key、不访问模型网络，不能把Mock成绩宣传为真人模型成绩。
- 证据：`evaluation/results/mock-latest.json`、`docs/evaluation/report.md`；指标专项与固定回归共11条测试通过。
...
- RED：新增当前版本截图优先的浏览器审计后，真实暴露选宠页缺少一级主标题、Demo弹窗焦点／Escape行为缺失、首屏图片LCP警告，以及Manifest只有`any`图标尺寸四类问题。
- GREEN：补齐动态一级标题；弹窗打开后聚焦关闭按钮、Escape关闭并把焦点还给原按钮；只对首屏图片启用优先加载；新增由现有港湾插画生成的192px和512px标准PWA图标。
- VERIFY：七个主要页面在390×844手机和1280×900桌面Chromium中完成结构、导航、替代文字、控件名称、控制台和网络审计；320／390／430px手机宽度无页面级横向溢出；五轮中文输入、推荐结果和弹窗键盘路径通过。T42专项结果为9条通过、1条按项目条件跳过；前端全量53条单元／组件测试及25条浏览器路径通过，另有1条按桌面项目条件跳过，Lint、类型、OpenAPI漂移检查和生产构建通过。
- 证据：`frontend/e2e/t42-audit.spec.ts`、`docs/qa/browser-check.md`、`docs/qa/screenshots/t42/`。
- 边界：Manifest与普通链接回退已由浏览器验证；实体手机系统安装动作仍待完整前后端部署到可访问HTTPS地址后手工确认。T42未调用DeepSeek，未更新公开网址。
...
- 文档：补齐通俗技术架构与Mermaid流程图，扩充关键决策记录，并把README、演示脚本和固定评测报告统一到当前真实实现与证据边界。
- TDD修复：新增报告生成器回归测试，先复现旧`SDD_SPEC 0.3.8`和“待T42执行”会覆盖正确报告，再让生成器固定写入`SDD_SPEC 0.3.10`与T42补充审查结果。
- VERIFY：README的新命令实际执行38条关联测试并全部通过；本地Markdown链接有效，3个Mermaid图由CLI实际渲染成功，关键术语与规格、代码及T41/T42证据一致。
- 边界：没有修改生产业务行为，没有调用DeepSeek，没有部署或推送；实体手机系统安装仍保留为T44之后的手工项。
...
**T44完成记录（技术验证：2026-09-11；用户验收：2026-09-12）：**

- RED：首次全量验证在mypy阶段发现评测测试文件无法解析仓库根`evaluation`模块；报告索引回归测试也先复现“最终报告尚未生成”的旧文案。
- GREEN：为mypy声明仓库根模块路径，修正验证脚本阶段文案，并让报告生成器稳定引用`docs/qa/final-verification.md`。
- VERIFY：`scripts/verify.ps1`全绿；前端53条单元／组件测试和25条浏览器路径通过，1条按桌面项目条件跳过；后端234条测试通过、总覆盖率95%；Ruff、mypy、OpenAPI漂移、依赖锁与Next.js生产构建通过；固定评测关联38条测试通过。
- 证据：`docs/qa/final-verification.md`、`docs/evaluation/report.md`、`docs/qa/browser-check.md`。
- 边界：未调用DeepSeek，未推送GitHub，未更新公开网址；实体手机系统安装与T35B接线后的真人完整复跑仍明确保留。
- 用户验收：用户于2026-09-12明确批准T44验收；第一版本地开发正式完成。该批准不等于授权推送GitHub、更新公开网址或部署完整后端。

**检查点 V / Phase 8出口：** 已通过用户验收，形成可展示的第一版本地Demo；部署和公开仓库更新仍需另行批准。
...
- T42：`frontend/e2e/t42-audit.spec.ts`
...
UX-01最终版已于2026-08-12获批，T24至T26已按TDD完成，Phase 4检查点M通过。T27至T32和Phase 5检查点P均已完成；T33 DeepSeek适配器与T34一次受控重试和失败回滚均已通过假HTTP测试；T35已使用`DeepSeek-V4-Flash-0731`完成5轮受控真人流程，Phase 6检查点R通过；T36至T39及Phase 7检查点T已完成，T39已同步到公开分享版；T40至T43已完成固定评测、浏览器审查与展示材料；T44最终全量技术验证已通过并形成`docs/qa/final-verification.md`，且于2026-09-12通过用户验收，Phase 8检查点V通过，第一版本地开发正式完成。自动化未调用DeepSeek或其他网络模型，真人结果单独记录。实体手机系统安装与T35B接线后的真人完整复跑仍是明确保留项；公开GitHub仓库已存在，但T35B及T40至T44改动尚未推送，公开网址仍是前端原型分享版，正式部署和外部付费仍需单独批准。
