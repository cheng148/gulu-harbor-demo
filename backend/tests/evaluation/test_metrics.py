...
def test_report_separates_mock_results_from_deferred_browser_checks() -> None:
    observations = tuple(
    ...
    assert "T42真实浏览器审查" in report
    assert "样本总数 | 36" in report
    assert "已执行 | 34" in report
    assert "待T42执行 | 2" in report
    assert "真人模型指标不得用本报告冒充" in report
    assert "全站明显按钮由T42真实浏览器审查" in report
