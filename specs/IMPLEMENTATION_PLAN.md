...
- 创建日期：2026-08-04
- 批准日期：2026-08-04
- 输入规格：`specs/SDD_SPEC.md` 0.2.0（Gate 0已批准）
- 当前阶段：PLAN完成；第一批Phase 0已完成并验证
- 门禁：第二批Phase 1未获批准前不得实施T06及后续业务任务
...
当前只有Markdown规格、协作规则和历史PDF，没有`package.json`、`pyproject.toml`、锁文件或生产代码。因此Gate 1不能“沿用现有依赖版本”，只能定义版本选择和锁定方法。
...
检索结果会进入解释模型上下文，因此仍是“检索增强生成”；但候选选择和证据来源可完全测试。以12个方向和24个具体档案的规模，引入向量数据库会增加依赖、随机性和解释困难，收益不足。
...
- 先写失败测试，再实现状态Schema、槽位合并、硬约束、冲突、问题排序、评分、等级和公共去分。
- 建立方向、宠物和知识Schema及最小测试数据。
...
Phase 3至5后完成Gate 3候选验收。Mock端到端未稳定，不接DeepSeek。
