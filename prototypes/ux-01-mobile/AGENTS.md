...
## Approved UX-01 visual target

- Use the user-selected original pre-recolor harbor scrapbook mobile mock as the visual source of truth.
- Preserve warm paper, deep harbor navy, coral emphasis, cat-and-dog lighthouse imagery, handwritten display type, and bottom navigation.
- Do not use the three later recolor variants. Keep interactions mock-only and isolated from production frontend code.
- The selected harbor screen is the AI pet-selection entry, not the overall marketplace home. Highlight the pet-selection tab on this screen.
...
## Confirmed discovery homepage

- The App homepage is a Taobao-like discovery feed with search, category filters, an AI advisor entry, recommended pets, and partner shops.
...
- 选宠对话页的顶部栏固定在设备视口顶部，不随聊天内容滚动；顶部仅保留左侧返回和阶段标题。“重新开始”放在聊天内容最下方，使用单行弱按钮，并保留二次确认。
...
- 首页、选宠、社区、我的必须共用同一条固定底栏和相同几何尺寸；“选宠”在任何标签状态下都始终使用动物脚印图标。
- 真实触屏手机优先调用系统输入法以支持用户安装的中文输入法；桌面设备预览保留可点击的模拟键盘。